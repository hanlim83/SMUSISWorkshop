...
public class jobLocationFullTimeController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        thread = new Thread(() -> {
            Shell.run(SelectEquals.class, "-i data/data_salary.csv -o data/data_filtered.csv -c 4 -e Full-Time");
            Shell.run(ProjectColumns.class, "-i data/data_filtered.csv -o data/data_countries.csv -c location meanSalary");
    ...
    void generateIndonesiaRegression(ActionEvent event) {
        ...
        thread = new Thread(() -> {
            ...
            Shell.run(PlotLinearRegression.class, "-i data/data_Indonesia.csv -m model/jobCountriesIndonesia.model -n Indonesia(Full-Time)");
            Platform.runLater(() -> {
                hamburger.setDisable(false);
                singaporeBtn.setDisable(false);
                malaysiaBtn.setDisable(false);
    ...
    void generateMalaysiaRegression(ActionEvent event) {
        ...
        thread = new Thread(() -> {
            ...
            Shell.run(PlotLinearRegression.class, "-i data/data_Malaysia.csv -m model/jobCountriesMalaysia.model -n Malaysia(Full-Time) ");
            Platform.runLater(() -> {
                hamburger.setDisable(false);
                singaporeBtn.setDisable(false);
                malaysiaBtn.setDisable(false);
    ...
    void generatePhilippinesRegression(ActionEvent event) {
        ...
        thread = new Thread(() -> {
            ...
            Shell.run(PlotLinearRegression.class, "-i data/data_Philippines.csv -m model/jobCountriesPhilippines.model -n Philippines(Full-Time)");
            Platform.runLater(() -> {
                hamburger.setDisable(false);
                singaporeBtn.setDisable(false);
                malaysiaBtn.setDisable(false);
    ...
    void generateSingaporeRegression(ActionEvent event) {
        ...
        thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Shell.run(PlotLinearRegression.class, "-i data/data_Singapore.csv -m model/jobCountriesSingapore.model -n Singapore(Full-Time)");
