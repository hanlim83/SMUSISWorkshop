...
import ai.preferred.crawler.jobsCrawler;
...
public class jobListingHandler implements Handler {
    private static final Logger LOGGER = LoggerFactory.getLogger(jobListingHandler.class);
    @Override
    public void handle(Request request, VResponse vResponse, Scheduler scheduler, Session session, Worker worker) {
        ...
        final EntityCSVStorage<jobListing> storage = session.get(jobsCrawler.STORAGE_KEY);
